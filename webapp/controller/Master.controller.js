...
sap.ui.define([
	...
	return BaseController.extend("appay.AP_PayRequest.controller.Master", {
		...
		onInit: function () {
			...
			var oList = this.getView().byId(sap.ui.core.Fragment.createId("frgSharedMasterListAll", "documentsList")),
				oListSaved = this.getView().byId(sap.ui.core.Fragment.createId("frgSharedMasterListSaved", "documentsList")),
				oListSubmitted = this.getView().byId(sap.ui.core.Fragment.createId("frgSharedMasterListSubmitted", "documentsList")),
				oListRejected = this.getView().byId(sap.ui.core.Fragment.createId("frgSharedMasterListRejected", "documentsList")),
			...
			this._oList = oList;
			this._oListSaved = oListSaved;
			this._oListSubmitted = oListSubmitted;
			this._oListRejected = oListRejected;
		...
		onAfterRendering: function () {
			var oSemanticPage = this.getView().byId("page");
			var oAddAction = oSemanticPage.getAddAction();
			var oButton = oAddAction.getAggregation("_control");
			oButton.setText("Create New Document");
		...
		onUpdateFinished: function (oEvent) {
			...
			var recordsTotal = this._updateListItemCount(oEvent.getParameter("total"));
			// hide pull to refresh if necessary
			this.byId("pullToRefresh").hide();

			if (oEvent.getParameters().id.search("frgSharedMasterListAll") > 0) {
				this._findItem();
				this.getModel("appView").setProperty("/addEnabled", true);
			}

			// if (oEvent.getParameters().id.search("frgSharedMasterListAll") > 0) {
			// 	this.getModel("appView").setProperty("/totalRecords", recordsTotal);
			// } else if (oEvent.getParameters().id.search("frgSharedMasterListSaved") > 0) {
			// 	this.getModel("appView").setProperty("/savedRecords", recordsTotal);
			// } else if (oEvent.getParameters().id.search("frgSharedMasterListSubmitted") > 0) {
			// 	this.getModel("appView").setProperty("/submittedRecords", recordsTotal);
			// } else if (oEvent.getParameters().id.search("frgSharedMasterListRejected") > 0) {
			// 	this.getModel("appView").setProperty("/RejectedRecords", recordsTotal);
			// }
		...
		onAdd: function () {
			...
			var draftDocument = undefined;
			for (var objectIndex in oData) {
				if (oData[objectIndex].Belnr === "9999999999" &&
					oData[objectIndex].__metadata.type === "ZFAP_PAY_REQ_SRV.ET_FV60Header") {
					draftDocument = oData[objectIndex];
				}
			}
			if (draftDocument !== undefined) {
				var sObjectPath = draftDocument.__metadata.uri.slice(draftDocument.__metadata.uri.search("/ET_FV60"));
		...
		_showDetail: function (oItem) {
			...
			if (oItem.getBindingContext().getProperty("Belnr") !== "9999999999" ||
				this.getModel("appView").getProperty("/cancelButtonPressed")) {
		...
		_applyFilterSearch: function () {
			var sSelectedKey = this.getView().byId("masterFilterBar").getSelectedKey();

			var aFilters = this._oListFilterState.aSearch.concat(this._oListFilterState.aFilter),
				oViewModel = this.getModel("masterView");
			if (sSelectedKey === "All") {
				this._oList.getBinding("items").filter(aFilters, "Application");
			} else if (sSelectedKey === "Saved") {
				aFilters.push(new Filter("ZdocStatus", "EQ", "SAVED"));
				this._oListSaved.getBinding("items").filter(aFilters, "Application");
			} else if (sSelectedKey === "Submitted") {
				aFilters.push(new Filter("ZdocStatus", "EQ", "SUBMITTED"));
				this._oListSubmitted.getBinding("items").filter(aFilters, "Application");
			} else if (sSelectedKey === "Rejected") {
				aFilters.push(new Filter("ZdocStatus", "EQ", "REJECTED"));
				this._oListRejected.getBinding("items").filter(aFilters, "Application");
			}
