sap.ui.define([
	...
	function getFrameUrl(sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("appay/AP_PayRequest/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#APPayRequest-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#APPayRequest-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("appay.AP_PayRequest.test.integration.pages.Common", {
		...
		getMockServer: function () {
			return new Promise(function (success) {
				Opa5.getWindow().sap.ui.require(["appay/AP_PayRequest/localService/mockserver"], function (mockserver) {
